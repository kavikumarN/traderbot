...
async def test_initialize_raises_when_quantity_missing() -> None:
    plugin = MacdStrategy(make_context(fast_period=2, slow_period=5, signal_period=2))
...
async def test_on_tick_is_a_no_op() -> None:
    plugin = MacdStrategy(make_context(quantity="1", fast_period=2, slow_period=5, signal_period=2))
    await plugin.initialize()
...
async def test_crossovers_emit_signals_and_unchanged_relation_emits_nothing() -> None:
    fast_period, slow_period, signal_period = 2, 5, 2
    prices = [Decimal(v) for v in [100, 100, 100, 100, 100, 130, 150, 170, 60, 40, 20]]

    macd = MacdIndicator(fast_period=fast_period, slow_period=slow_period, signal_period=signal_period)
    previous_relation: int | None = None
    expected: dict[int, OrderSide] = {}
    for i, price in enumerate(prices):
        macd_line, signal_line = macd.update(price)
    ...
    plugin = MacdStrategy(
        make_context(quantity="1", fast_period=fast_period, slow_period=slow_period, signal_period=signal_period)
    )
    await plugin.initialize()
